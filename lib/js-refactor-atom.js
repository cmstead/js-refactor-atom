...
export default {
    ...
    activate(state) {
        ...
        let reformat = this.callWithEditor(reformatAction).bind(null, this.config);
    ...
    callWithEditor(action, callback) {
        return function(defaults) {
            ...
            let config = atom.workspace.config.settings['js-refactor-atom'];

            if (editor) {
                action(editor, callback, config, defaults);
            }
        };
    }
};
