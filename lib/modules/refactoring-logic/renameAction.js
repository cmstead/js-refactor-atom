...
function renameAction(scopeFinder) {
    ...
    function performRename(fileSource, functionSource, varName, newName) {
        let pattern = new RegExp('(\\\.?\\\s*[^\\\w_$])' + varName + '([^\\\w_$])', 'g');
        let updatedSource = functionSource.replace(pattern, function(str, p1, p2) {
            return str.startsWith('.') ? str : p1 + newName + p2;
        });
    ...
    function adjustNameSelection(selectionCoords) {
        return {
            start: [selectionCoords.start[0], selectionCoords.start[1]],
            end: [selectionCoords.end[0], selectionCoords.end[1] - 1]
        };
    }
